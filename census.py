...
names = [
    "k.sriharsha", "j.devi varshini ", "G.ramya",
...
url = 'https://raw.githubusercontent.com/Rahul6158/census/ff0a9cddb8c83f4769f5c8ac32b0a295576264f3/IndiaCensus2011.csv'
data = pd.read_csv(url)

st.title("India Census Data Analysis")

if st.checkbox("Checking whether the data is preprocessed or NOT (Any NULL Values)"):
...
if st.checkbox("SOME COLUMN OPERATIONS"):
    ...
    elif option == "Set a column as an index":
...
if st.checkbox("SOME STATISTICAL OPERATIONS"):
    option = st.radio(
        ...
        ('Statistics of the dataset ?',
         ...
         'Calculate the total population of India according to the 2011 Census ?',
    ...
    if option == 'Calculate state-wise total number of population and population with different religions':
        st.write(data.groupby('State_name').agg({
            'Population': 'sum', 'Hindus': 'sum', 'Muslims': 'sum', 'Christians': 'sum',
            'Sikhs': 'sum', 'Buddhists': 'sum', 'Jains': 'sum'
    ...
    if option == 'Which state has the highest population ?':
        ...
        st.write(f"{highest_population.index[0]} has the highest population of {highest_population['Population'][0]}. It is because the number of districts in Uttar Pradesh is more")
...
if st.header("\nData visualizations"):
    if st.checkbox("Show the percentages of Sikhs, Christians, Jains, Buddhists in India by a pie chart"):
        st.write()
